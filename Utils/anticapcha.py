...
# обеспечение многопоточной работы с классом
class RUCaptchaThreading(threading.Thread):
    ...
    def __check_values(self):
        ...
        for item in self.__queue:
            ...
            url = "http://rucaptcha.com/res.php?key={apikey}&action=get&id={captcha_id}".format(apikey=self.__apikey,
            ...
            if r.status_code == 200:
                if "CAPCHA_NOT_READY" in r.content:
                    ...
                    print("Unknown response: {response!r}".format(response=r.content))
                    #
...
class RUCaptchaValue(object):
    ...
    def __repr__(self):
        result = "<RUCaptchaValue is_ready={ready!r} value={value!r} expire={expire!r}>".format(ready=self.__ready,
                                                                                                value=self.__value,
                                                                                                expire=self.is_expire())
        return result
...
class RUCaptcha(object):
    ...
    def parse(self, path, is_phrase=0, is_regsense=0, is_numeric=0, min_len=0, max_len=30, language=0,
        ...
        if r.status_code == 200:
            result = self.__check_response(r.content)
        return result


def crack_capcha(apikey, path):
    """
    декодирует капчу
    Аргументы:
        apikey    ключ для сервиса rucapcha
        path      путь к дедодируемой капче
    """
    ru_captcha = RUCaptcha(apikey=apikey)  # инициализация обьекта RUcapcha
    value = ru_captcha.parse(path=path, is_regsense=1)  # отсылаем запрос

    print("{value!r}".format(value=value))

    while not value.is_ready():  # ожидаем ответ в цикле
        time.sleep(0.1)

    print("{value!r}".format(value=value))

    res = value.get_value()  # по получении ответа парсим нужные данные

    ru_captcha.dispose()  # освобождаем ресурсы
